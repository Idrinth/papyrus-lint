...
- `rules`: per-lint enable/disable switches, one key per rule (the key name
  ...
  that have no way to appear in `shared/rules/data/skyrim/actor-values.yaml`, most
  ...
  mismatch" lint's `shared/rules/data/skyrim/known-events.yaml` only lists a curated

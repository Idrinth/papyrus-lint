...
copying the logos and screenshots from `shared/images/`, rather than committing
duplicate copies of them under `pages/`. For the five PNG screenshots rendered
as `<img>` elements, it additionally
writes a WebP and an AVIF sibling next to the copied original. The SVG header
logo is copied directly, while `logo.png` is only referenced as a raw
`og:image`/`twitter:image` URL and the favicon is only referenced via
`<link rel="icon">`. The modern screenshot variants are generated by
`site_assets.convert_to_modern_formats` via `Pillow` (see
`pages/requirements-build.txt`), losslessly so text and lines stay crisp;
`site_assets.wrap_images_with_modern_sources` then rewrites
every such

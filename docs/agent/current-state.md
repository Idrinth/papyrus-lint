...
baseline `presets::Preset` (`papyrus-lint-core/src/presets.rs`) it generates
...
`presets::save_user_preset`, which writes just the lint settings (not a
...
- **Rename** (`handleRenamePresetClick`) prompts for a new name, applying
  ...
  (`papyrus_lint_core::presets::rename_user_preset`), which finds the
...
`presets::preset_lint_config_default` — a sibling of

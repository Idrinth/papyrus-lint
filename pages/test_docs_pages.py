...
class PublishedSchemaTest(unittest.TestCase):
    def test_ai_export_rule_details_expose_auto_fixability(self) -> None:
        schema = json.loads(
            (docs_pages.SCHEMA_DIR / "papyrus-lint-ai-export.v4.schema.json").read_text(encoding="utf-8")
        )
...
class ResolveDocHrefTest(unittest.TestCase):
    def test_handles_docs_repository_and_external_links(self) -> None:
        with (
            ...
            self.assertEqual(
                docs_pages.resolve_doc_href("../shared/rules/data/skyrim/example.yaml"),
                "https://example.test/repository/shared/rules/data/skyrim/example.yaml",
            )

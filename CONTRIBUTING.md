...
How to install and run the desktop app and the editor plugins lives in
...
prerequisites for a desktop build). Each crate's own test command lives
in that crate's `README.md`. Do not recopy either here.
...
- **Rust test job**: a matrix over `app/src-tauri`, `app/crates/papyrus-parser`,
  ...
  `app/crates/papyrus-lint-output`, `app/crates/papyrus-lint-live`,
  `app/crates/papyrus-lint-lsp`, and

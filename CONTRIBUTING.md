...
crate::visitor::LintVisitor`. `collect_diagnostics` registers those onto
the AST/token walkers, walks both trees once, and emits the diagnostics
the visitors collected. `check` for those rules is that same walk for a
single visitor. `Rules`,

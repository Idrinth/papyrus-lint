...
register it with a `mod` in `app/crates/papyrus-lints/src/lib.rs`, an
entry in [`docs/rules.json`](docs/rules.json), and a row in
`app/crates/papyrus-lints/rule_dispatch.json` (`check`, plus `repair` /
`repair_order` if `registry::apply_repairs` should auto-fix it). `Rules`,
...
the `docs/rules.json` entry for opt-in rules.
...
and update `docs/rules.json`, `rule_dispatch.json`, and the configuration
examples (`docs/papyrus-lint.default.yaml`, `docs/nexuspage.bbcode`).
`registry.rs`'s `KNOWN_RULE_IDS`/`FIXABLE_RULE_IDS`, `tags.rs`'s
`RULE_TAGS`, `config.rs`'s `Rules`, and the check/repair dispatch are all
compiled from `docs/rules.json` / `rule_dispatch.json` by `build.rs`, so they

...
entry in [`docs/rules.json`](docs/rules.json) (`repair_order` if
...
and update `docs/rules.json` and the configuration
...
compiled from `docs/rules.json` by `build.rs`, so they

...
register it with a `mod` in `app/crates/papyrus-lints/src/lib.rs` and an
entry in [`shared/rules.json`](shared/rules.json) (`repair_order` if
`registry::apply_repairs` should auto-fix it). Every source-level check is
...
the `shared/rules.json` entry for opt-in rules.
...
compiled from `shared/rules.json` by `build.rs`, so they

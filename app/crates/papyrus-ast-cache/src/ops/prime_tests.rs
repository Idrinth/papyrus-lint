...
use crate::ops::store::put_in_for_game;
use crate::ops::test_support::{harness, sample_ast, sample_tokens, COMPATIBLE_VERSION, GAME};
...
fn ensure_primed_is_a_noop_on_an_already_cached_entry() {
    ...
    let first_ast = get_in_for_game(cache_dir.path(), GAME, &source_path, source);
    let first_tokens = get_tokens_in_for_game(cache_dir.path(), GAME, &source_path, source);
    ...
    let second_ast = get_in_for_game(cache_dir.path(), GAME, &source_path, source);
    let second_tokens = get_tokens_in_for_game(cache_dir.path(), GAME, &source_path, source);
...
fn ensure_primed_fills_in_a_missing_ast_field_without_disturbing_already_cached_tokens() {
    ...
    crate::ops::store::put_tokens_in_for_game(
        cache_dir.path(),
        GAME,
        &source_path,
        source,
...
fn ensure_primed_rewrites_after_the_source_content_changes() {
    ...
    assert_eq!(
        get_in_for_game(cache_dir.path(), GAME, &source_path, original),
...
fn ensure_primed_rewrites_after_the_file_mtime_changes() {
    ...
    assert!(get_in_for_game(h.cache_dir.path(), GAME, &h.source_path, h.source).is_some());
...
fn ensure_primed_writes_nothing_when_the_source_does_not_tokenize() {
    ...
    assert!(!cache_file_path_for_game(h.cache_dir.path(), GAME, &h.source_path).exists());
}

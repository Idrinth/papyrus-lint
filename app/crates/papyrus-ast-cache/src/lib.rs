...
//! it embeds in a way that would break reading older entries.
//!
...
//! [`version::MIN_COMPATIBLE_VERSION`], and [`ops`] is the `get`/`put`/
//! `ensure_primed` logic built on top of both, parameterized over a cache
//! directory so it can be tested without touching the real one. This file
//! wraps [`ops`] with [`CACHE_LOCK`] and the real cache directory to form
...
/// mismatch, or error -- the caller should parse `source` fresh in that
/// case. See [`ops::get_in`] for the in-memory priming a hit also does.
...
/// case. See [`ops::get_tokens_in`] for the in-memory priming a hit also
/// does.
...
/// runs. A disk cache hit for either already primes the matching in-memory
/// cache as a side effect; a miss for either parses/tokenizes `source` once
/// here instead (which populates the in-memory cache the same way a hit
/// would) and writes the result to the disk cache for next time. Used by
/// the desktop app's `lint_psc_file`/`repair_psc_file` commands and the
/// CLI's own per-script lint loop, so relinting an unchanged script --
/// across separate desktop app commands or CLI invocations -- skips both
/// re-parsing and re-tokenizing it there too, not just in `get`/
/// `get_tokens`'s other existing callers. See [`ops::ensure_primed_in`].

...
impl FunctionTable {
    ...
    /// case-insensitively. Base-game types are resolved from the bundled
    /// script cache when their `.psc` is not present in the project.
    ...
    /// a definite root: a project or bundled script found (and parsed) with
    /// no `Extends` line at all. Matched case-insensitively, mirroring
    ...
    /// along the way this table has no project or bundled script for means
    /// the ancestry is *not*

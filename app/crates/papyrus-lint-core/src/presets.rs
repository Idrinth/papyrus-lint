...
//! [`preset_lint_config`] apply on top of it), the user-preset management
...
//! [`list_user_preset_names`]), and [`PresetInfo`]/[`all`] — the label/
//! description text a picker renders for each one, for both the three
//! built-ins and any user preset found under [`user_presets_dir`]. Loading/
//! saving a project's own `papyrus-lint.yaml` (not a preset) is
//! [`crate::config`]'s job; this module borrows a few of its private helpers
//! (project-file (de)serialization, the executable-adjacent directory
//! lookup) rather than duplicating them.
...
/// executable-adjacent `presets` directory (see [`user_presets_dir`]), in
/// alphabetical order.
pub fn all() -> Vec<PresetInfo> {
    all_with_user_presets_dir(user_presets_dir().as_deref())
}
...
fn all_with_user_presets_dir(dir: Option<&Path>) -> Vec<PresetInfo> {
    ...
    if let Some(dir) = dir {
        all.extend(list_user_preset_names(dir).into_iter().map(|name| PresetInfo {
            id: name.clone(),
            label: name.clone(),
            description: format!(
...
mod tests {
    ...
    use super::*;
    use crate::config::{config_file_path, load_config_from_path};

...
fn repair_with_rules(source: &str, rules: &'static [SlowFunctionRule]) -> String {
    ...
    for (call_index, token) in tokens.iter().enumerate() {
        ...
        let start = token_offset(source, &line_starts, token);
        let argument_start = token_offset(source, &line_starts, &tokens[call_index + 1]) + 1;
        let close = token_offset(source, &line_starts, &tokens[close_index]);
        let end = close + 1;
        let argument = source[argument_start..close].trim();
        ...
        let argument = repair_with_rules(argument, rules);
...
mod tests {
    ...
    fn repairs_getter_with_the_complete_replacement_expression() {
        let source = "value = akGlobal.GetValueInt()\n";

        assert_eq!(repair(source), "value = akGlobal.GetValue() As Int\n");
    }

    #[test]
    fn repairs_setter_and_preserves_its_argument_expression() {
        let source = "akGlobal.SetValueInt(GetAmount(1, 2) + 3)\n";

...
fn an_empty_yaml_document_uses_the_complete_default_configuration() {
    let from_yaml: Config = serde_norway::from_str("").expect("empty config should be valid");
...
fn invalid_yaml_enum_values_are_rejected_instead_of_silently_defaulting() {
    let error = serde_norway::from_str::<Config>("indentation: spaces\n")
...
fn bool_like_int_setting_changes_strict_boolean_results() {
    ...
    let strict_config: Config =
        serde_norway::from_str("bool_like_int: false\n").expect("bool_like_int should deserialize");
...
fn assume_auto_properties_filled_setting_changes_none_form_usage_results() {
    ...
    let assume_filled_config: Config =
        serde_norway::from_str("assume_auto_properties_filled: true\n")
...
fn config_round_trip_preserves_user_visible_settings_and_rule_switches() {
    ...
    let round_tripped: Config =
        serde_norway::from_str(&serialized).expect("serialized config should deserialize");

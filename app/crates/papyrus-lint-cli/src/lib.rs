...
use args::{parse_cli, write_parse_error, LintArgs, ParsedCli, SharedFlags};
use blob::run_blob;
use doctor::run_doctor;
use init::{initialize_config, report_add_user_preset};
use output::*;
use project::*;
...
pub fn run(
    ...
    match parse_cli(args) {
        Err(err) => {
            write_parse_error(&err, stderr);
            return 2;
        }
        Ok(ParsedCli::Init { preset }) => {
        ...
        Ok(ParsedCli::Doctor { rest }) => {
            return run_doctor(&rest, stdout, stderr);
        }
        Ok(ParsedCli::Version) => {
            let _ = writeln!(stdout, "PapyrusLinterCLI {VERSION}");
            return 0;
        }
        Ok(ParsedCli::Blob(blob)) => {
            return run_blob(
                &blob.source,
                blob.flags.config_path.as_deref(),
                blob.flags.output_format,
                blob.flags.hash_source,
                blob.flags.quiet_warnings,
                blob.flags.quiet_info,
                blob.flags.tag_filter.as_deref(),
                blob.flags.color_choice,
                blob.flags.output_path.as_deref(),
                stdout_is_terminal,
                stdout,
                stderr,
            );
        }
        Ok(ParsedCli::Lint(lint)) => lint,
    };

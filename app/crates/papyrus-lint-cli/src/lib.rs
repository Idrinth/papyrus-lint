...
/// [`run_blob`], so both report the exact same "unknown tag" error.
fn normalize_tag_filter(tag_filter: Option<String>) -> Result<Option<String>, String> {
    match tag_filter {
        Some(value) => {
...
fn run_doctor(args: &[String], stdout: &mut impl Write, stderr: &mut impl Write) -> u8 {
    let mut json = false;
    let mut config_path: Option<PathBuf> = None;
    let mut cli_script_roots: Vec<String> = Vec::new();
    let mut positionals: Vec<String> = Vec::new();

...
fn blob_mode_lints_unsaved_source_through_the_binary_entry_point() {
    ...
    assert!(output.stderr.is_empty());
    let stdout = String::from_utf8(output.stdout).expect("stdout should be UTF-8");
    assert!(stdout.contains("<blob>:1:19: [trailing-whitespace]"));
...
#[test]
fn blob_mode_rejects_combining_inline_source_with_a_path() {
    let dir = tempfile::tempdir().expect("failed to create temp directory");
    let script = dir.path().join("Example.psc");
    write_file(&script, "ScriptName Example\n");

    let output = run_cli(&["--blob", "ScriptName Unsaved\n", &script.to_string_lossy()]);

...
| **ScriptName/filename mismatch** | Flags, as an `[error]`, a `.psc` file whose declared `ScriptName` doesn't match its own file name, aside from casing (e.g. `ScriptName Example` in a file named `Other.psc`), since Papyrus resolves/compiles a script by matching the two and rejects a mismatch at compile time. Only available when linting a file with a known path in the desktop app or CLI. | |
...
`default-property-value`, `unguarded-self-recursion`, and `self-assignment`.
...
- `rules`: per-lint enable/disable switches. Setting one to `false` turns
  ...
  `script_name_collision`, `array_bounds`, `readonly_property_write`,
  `default_property_value`, `unguarded_self_recursion`, and
  `self_assignment`.

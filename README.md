...
- `rules`: per-lint enable/disable switches. Setting one to `false` turns
  ...
  `native_function_usage`, and `global_variable_setvalue`, which default to
  ...
  need to be warned about it, and the `GlobalVariable` no-op write lint's
  `Else`-branch case is a heuristic rather than a proven no-op. The key
  names match the lints listed above:
  ...
  `magic_numbers`, `native_function_usage`, and `global_variable_setvalue`.

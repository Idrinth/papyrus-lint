...
class CoverageSummaryTests(unittest.TestCase):
    ...
    def test_parse_lcov_sums_records_and_tolerates_non_utf8_text(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual((15, 10, 4), coverage_summary.parse_lcov(report))
    ...
    def test_parse_lcov_ignores_other_numeric_fields(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual((5, 4, 9), coverage_summary.parse_lcov(report))
    ...
    def test_parse_lcov_accepts_an_empty_report(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual((0, 0, 0), coverage_summary.parse_lcov(report))
    ...
    def test_crap_estimate_combines_average_function_size_and_coverage(self) -> None:
        self.assertAlmostEqual(2.0625, coverage_summary.crap_estimate(4, 3, 2))
        self.assertAlmostEqual(2.0, coverage_summary.crap_estimate(2, 2, 1))

    def test_crap_estimate_returns_none_without_enough_data(self) -> None:
        self.assertIsNone(coverage_summary.crap_estimate(0, 0, 0))
        self.assertIsNone(coverage_summary.crap_estimate(4, 3, 0))

    def test_format_crap_renders_one_decimal_or_na(self) -> None:
        self.assertEqual("n/a", coverage_summary.format_crap(None))
        self.assertEqual("2.1", coverage_summary.format_crap(2.0625))
    ...
    def test_render_entry_includes_nested_group_totals_and_leaf_rows(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            found, hit, functions_found, rows = coverage_summary.render_entry(root, "Group", value, 0)

...
class BbcodeLintTests(unittest.TestCase):
    def messages(self, text: str) -> list[str]:
        return [issue.message for issue in bbcode.lint(text)]
    ...
    def test_accepts_every_supported_tag(self) -> None:
        text = "".join(f"[{tag}]content[/{tag}]" for tag in sorted(bbcode.KNOWN_TAGS))
    ...
    def test_reports_multiple_unclosed_tags_in_source_order(self) -> None:
        issues = bbcode.lint("before [b]bold [i]italic")
    ...
    def test_unknown_tags_do_not_affect_known_tag_nesting(self) -> None:
        self.assertEqual(
            ["unknown tag [wat]", "unknown tag [wat]"],
            self.messages("[b][wat]text[/wat][/b]"),
        )
    ...
    def test_location_reports_one_based_line_and_column(self) -> None:
        text = "first line\nsecond [b]line"
        self.assertEqual((2, 8), bbcode.location(text, text.index("[b]")))

    def test_location_handles_the_start_and_end_of_a_line(self) -> None:
        self.assertEqual((1, 1), bbcode.location("text", 0))
        self.assertEqual((2, 1), bbcode.location("first\n", 6))
    ...
    def test_check_file_prints_each_issue_with_its_location(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with redirect_stdout(output):
                issue_count = bbcode.check_file(path)

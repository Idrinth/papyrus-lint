"""Aggregates lcov.info coverage reports by project module.

Extracted out of coverage_summary.py (which mixed this logic with its CLI
entrypoint) with no behavior change. Shared with render_nexuspage.py, which
also needs to sum coverage across the same module tree.
"""
